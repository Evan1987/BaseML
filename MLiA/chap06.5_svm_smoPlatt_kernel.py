...
class optStruct:
    ...
    def innerL(self,i):
        Ei = self.calcEk(i)
        if (self.labelMat[i]*Ei < -self.toler and self.alphas[i] < self.C) or         (self.labelMat[i]*Ei > self.toler and self.alphas[i] > 0):
            ...
            self.updateEk(i)
            # update b
            b1 = Ei +             self.labelMat[i]*(self.alphas[i]-alphaIOld)*self.K[i,i] +             self.labelMat[j]*(self.alphas[j]-alphaJOld)*self.K[i,j] +             self.b
            b2 = Ej +             self.labelMat[i]*(self.alphas[i]-alphaIOld)*self.K[i,j] +             self.labelMat[j]*(self.alphas[j]-alphaJOld)*self.K[j,j] +             self.b
...
dataMat,labelMat = loadDataSet(path+"testSetRBF.txt")
testDataMat,testLabelMat = loadDataSet(path+"testSetRBF2.txt")
oS = smoP(dataMat,labelMat,C=200,toler=0.0001,maxIter=10000,kTup=("rbf",1.3))
...
testResult = classify(oS,testDataMat)


# In[115]:

precisionCal(trainResult,actualLabelMat=labelMat)


# In[114]:

precisionCal(testResult,actualLabelMat=testLabelMat)
